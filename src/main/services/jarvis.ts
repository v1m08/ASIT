...
function briefing(): string {
  ...
  return [
    ...
    'APPEND EVERY ACTION YOU CAN ALREADY DECIDE ON, IN ONE WRITE. Executing an action takes milliseconds; the cost of a page task is the round-trip — you re-reading, re-thinking and re-writing the file. Six form fields in one append is one round-trip; one at a time is six, which is the difference between three seconds and thirty. Split only where a later step truly depends on what an earlier one produces (a new page, a menu that must open first) — wanting to check it worked is not a dependency; the result file reports every action separately, so scan it for failures afterwards.',
    'Verbs: {"action":"open","target":"<resourceId>|builtin-notes"} · {"action":"add_url","title","url"} · {"action":"unpin","target"} / {"action":"rename_pin","target","title"} / {"action":"reorder_pins","order":[...]} (tidy the pins on a workspace — unpin leaves the file on disk) · {"action":"add_questions","questions":[{q,a,choices?,correct_index?}]} · {"action":"generate_questions","sources":["file"],"mode":"generate|extract","count"} · {"action":"set_task","title?","priority?","due_date?","status?"} · {"action":"save_skill","name","content"} · {"action":"watch","label?|text?|gone_label?|gone_text?","page?","prompt?","skill?","timeout_min?"} · page interaction: {"action":"page_snapshot"} then {"action":"page_click","label"|"ref"} / page_fill / page_select / page_key {"key":"Ctrl+P"} / page_type / navigate {"url"} · {"action":"wait","ms"}.',
    ...
    'Check this list first: search the web (`search`), read the user mail (`fetch`), open and drive any page (`navigate` + `page_*`), make/open/list/delete workspaces (`create_workspace` / `open_workspace` / `list_workspaces` / `delete_workspace`), save a link (`add_url`), create a note (`add_note`), manage the to-do list (`add_todo` / `complete_todo` / `list_todos`), start a focus session (`start_focus`), tidy a cluttered workspace (`unpin` / `rename_pin` / `reorder_pins`), write notes (Edit/Write inside the workspace folder), remember a standing fact (`remember`), schedule future runs (`schedule`), or arm a `watch` for something that has not happened yet. Between those you can reach almost anything on this machine.',
