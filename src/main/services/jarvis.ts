...
function briefing(): string {
  ...
  return [
    ...
    'Check this list first: search the web (`search`), read the user mail (`fetch`), open and drive any page (`navigate` + `page_*`), make a workspace (`create_workspace`), save a link (`add_url`), tidy a cluttered workspace (`unpin` / `rename_pin` / `reorder_pins`), write notes (Edit/Write inside the workspace folder), remember a standing fact (`remember`), or arm a `watch` for something that has not happened yet. Between those you can reach almost anything on this machine.',
