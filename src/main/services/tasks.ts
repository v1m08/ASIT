...
export function writeClaudeMd(task: Task, resources: Resource[]): void {
  ...
  lines.push(
    ...
    '',
    '1. Append your action line(s) — batch related steps together.',
    '2. Read `.asit/actions-result.md`. The app writes your batch there with a PER-ACTION outcome ("clicked …" / "no element matching …") once it finishes (~2-3s). If your newest batch is not there yet, Read the file again.',
    '3. After any page-changing batch the app auto-waits for the page to settle and AUTO-REFRESHES `.asit/pages/` BEFORE writing the result — so once your batch appears, the page files are already fresh. Read them to decide the next step. You rarely need an explicit page_snapshot.',
    '4. Repeat: act → read result → read page → act. NEVER assume an action worked — check its outcome line. If a click found no element, try the label variant, a different label, or a keyboard path.',
    '5. Keep looping until the task is DONE. Do not stop halfway to narrate; finish, then summarize. If you must wait on something slow (video, user pressing Play, server boot), register a `watch` before ending your turn.',
    '6. YOU CANNOT WAIT by saying so. Statements like "I\'ll continue when X happens" without an ARMED watch are false and forbidden. Short pauses: {"action":"wait","ms":≤10000} between actions. Longer: a watch. There is no third option.',
    '7. After any multi-step page flow SUCCEEDS, save it via save_skill as an AUTO-FLOW (```asit-flow block) so the user can replay it instantly with ./name — that is the entire point of skills.',
    '',
    'Available commands:',
    '',
