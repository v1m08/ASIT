...
- `src/main/services/` — one service per domain: `tasks` (CRUD, folders, CLAUDE.md, scratchpad, privacy), `resources`, `panes` (WebContentsView manager + page bridge), `claude` (CLI spawn + NDJSON parse), `chat`, `assistant` (global haiku bar), `timer` (stopwatch/pomodoro state machine), `lockdown`, `questions` (job queue + SM-2), `actions` (AI app-control protocol), `accounts`, `library`, `transfer` (backup zip), `usage` (token/cost log), `settings`, `companion` (phone PWA server + web push), `bus` (in-process pub/sub so services never import each other for events)
...
- Headless smoke tests (after build): env `ASIT_SMOKE=1` | `ASIT_SMOKE_CHAT=1` | `ASIT_SMOKE_QGEN=1` | `ASIT_SMOKE_AGENT=1` | `ASIT_SMOKE_TRANSFER=1` | `ASIT_SMOKE_PANES=1` | `ASIT_SMOKE_COMPANION=1` with `npx electron out/main/index.js` (bottom of `src/main/index.ts`). Smoke runs are isolated: own userData (%APPDATA%\Electron) and documents redirected to temp — they must never touch real user data.
