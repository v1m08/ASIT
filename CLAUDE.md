...
- Headless smoke tests (after build): env `ASIT_SMOKE=1` | `ASIT_SMOKE_CHAT=1` | `ASIT_SMOKE_QGEN=1` | `ASIT_SMOKE_AGENT=1` | `ASIT_SMOKE_TRANSFER=1` | `ASIT_SMOKE_PANES=1` with `npx electron out/main/index.js` (bottom of `src/main/index.ts`). Smoke runs are isolated: own userData (%APPDATA%\Electron) and documents redirected to temp — they must never touch real user data.
